...
export function toUserDTO(u: AnyRecord | null): UserDTO | null {
  ...
  return {
    ...
    kycStatus: (u.kycStatus as string) ?? 'NONE',
    currency: (u.currency as string) ?? null,
    subRole: (u.subRole as string) ?? null,
    affiliateCode: (u.affiliateCode as string) ?? null,
    affiliateEarnings: (u.affiliateEarnings as number) ?? 0,
    planTier: (u.planTier as string) ?? null,
...
export async function toGoodDTO(g: AnyRecord, viewerId?: string | null): Promise<GoodDTO> {
  ...
  return {
    ...
    assetType: (g.assetType as GoodDTO['assetType']) ?? 'file',
    accessLink: (g.accessLink as string) ?? null,
    fileUrl: (g.fileUrl as string) ?? null,

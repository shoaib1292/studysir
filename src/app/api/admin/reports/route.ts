    ...
    const items: ReportDTO[] = await Promise.all(
      reports.map(async (r) => {
        ...
        if (r.targetType === 'GOOD' && r.targetId) {
          const g = await db.digitalGood.findUnique({ where: { id: r.targetId }, select: { title: true, image: true } })
          targetLabel = g?.title ?? null
          targetImage = g?.image ?? null
        } else if (r.targetType === 'COURSE' && r.targetId) {
          const c = await db.course.findUnique({ where: { id: r.targetId }, select: { title: true, cover: true } })
          targetLabel = c?.title ?? null
          targetImage = c?.cover ?? null
        } else if (r.targetType === 'TUITION' && r.targetId) {
          const t = await db.tuitionPost.findUnique({ where: { id: r.targetId }, select: { title: true } })
          targetLabel = t?.title ?? null
        }
        const dto = toReportDTO(r as never)
        return { ...dto, targetLabel, targetImage }
      })
    )
